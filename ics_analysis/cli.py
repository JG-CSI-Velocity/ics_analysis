"""Typer CLI entry point for ICS Analysis."""
...
from ics_analysis.exceptions import ICSError
from ics_analysis.settings import DEFAULT_CONFIG_PATH, Settings

app = typer.Typer(
    name="ics-analysis",
    help="ICS (Insured Cash Sweep) account analysis and reporting tool.",
...
def main(
    ...
    cohort_start: str = typer.Option(
        None,
        "--cohort-start",
        help="Cohort analysis start month (YYYY-MM format).",
    ),
    ics_not_in_dump: int = typer.Option(
        None,
        "--ics-not-in-dump",
        help="Count of ICS accounts not in the data dump.",
    ),
    ...
    Usage:
        python -m ics_analysis data/client_file.xlsx
        python -m ics_analysis --config config.yaml
        python -m ics_analysis data/client.xlsx --output ./reports/ --client-id 1453
    """
    ...
    try:
        ...
        if cohort_start is not None:
            overrides["cohort_start"] = cohort_start
        if ics_not_in_dump is not None:
            overrides["ics_not_in_dump"] = ics_not_in_dump
