...
from ics_analysis.analyses.base import safe_percentage
...
def grouped_summary(
    ...
    Args:
        ...
        pct_of: Column names to compute '% of {col}' as share of total (fraction 0-1).
...
def crosstab_summary(
    ...
    if add_rate_col and rate_numerator and rate_numerator in ct.columns:
        ct[add_rate_col] = np.where(
            ct["Total"] > 0,
            (ct[rate_numerator] / ct["Total"]).round(4),
            0.0,
        )
...
def append_grand_total_row(
    ...
    Sum columns get summed. Percentage columns get 1.0.
